  ...
  return (
    <div className="fixed inset-0 z-[1000] flex items-center justify-center bg-black/45 px-4">
      <div className="w-full max-w-sm rounded-lg bg-white p-6 text-center shadow-xl">
        <div className="mx-auto flex h-28 w-28 items-center justify-center rounded-full bg-[#EAF2FB] text-[#003366]">
          <Icon className="h-12 w-12" />
        </div>
        <h2 className="mt-5 text-lg font-bold text-[#030E18]">{step.title}</h2>
        <p className="mt-2 text-sm leading-6 text-[#657386]">{step.description}</p>

        <div className="mt-6 flex items-center justify-between">
          <button onClick={close} className="text-sm font-semibold text-[#657386] hover:text-[#003366]">
            Skip
          </button>
          ...
          <div className="flex gap-2">
            {index > 0 && (
              <button
                ...
                className="rounded-lg border border-[#E6EAF0] px-3 py-2 text-sm font-semibold text-[#263442]"
              >
                Back
              </button>
            )}
